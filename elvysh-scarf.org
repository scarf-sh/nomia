...
[fn:poly] Many of the concepts come together to form a particular kind of lax 2-multicategory.
*** Resources
...
[fn:0-cell] The 0-cells of the multicategory. Note that we do not in general identify a specific resource with some point of the relevant 0-cell, in part because there is no 1:1 mapping between a resource and its type and in part for reasons detailed in the next section.
*** Names
Each of the resources you utilized were referenced by a *name*[fn:1-cell]: "meld" names a particular program, "the test site for the PR you opened" names a particular web service. More generally, a name can be a procedure relating a finite sequence of resource types (its *inputs*) to a resulting resource type (its *output*)[fn:domcod]: "the Acme webservice" might be a name that relates inputs like an executable for the service, a database, and a service config file to an output web service. We can visualize that as:

[FIG]

Resource subtyping and containment can be captured in *conversions*, names that map a single input to a single output and are either operationally noops (*coercions*) or cheaply provide access to some aspect of the whole (*projections*). The server compliation process coerced the writeable file your editor was using to a readable stream to generate a bundle of both an updated server executable and an updated module, and then when you edited the second file your editor projected out the new module to fix the undefined reference warning.

Names must be *deterministic*, in the sense that the process must produce an equivalent output when provided equivalent inputs. This may seem to make them too strict to be useful, but there are two mitigating factors. First, recall that equivalence is a domain-specific notion; depending on how high level the output type equivalence is the name may have quite a bit of leeway in exactly how it instantiates the desired resource. Moreover, names can be *contextual* (or *indexical*), meaning that their output can depend on (some aspect of) the caller's context; "the results of today's prod pipeline run" depends on what "today" means. This is modelled by a *context* resource type at the input, which can be thought of as "the state of the world from some particular perspective"; it can conceptually be instantiated with a (unique) context instance from a caller outside the system or forwarded on (possibly after transformations) within it. Because each top-level instantiation is unique, contextual names are essentially unrestricted with respect to determinism, so long as the lack of determinism can be captured in the context.

Names with an empty list of inputs are called *named resources*[fn:points], since they correspond directly to the (single up to equivalence) resource produced when the name is run.

We can sequentially combine names via *substitution*[fn:1-comp], instantiating some input resources of one name with the outputs of some other names (and so on recursively), resulting in a new name. We might visualize "the Acme webservice using the executable compiled from the latest code, the pristine test db, and some provided config file" as:
...
[fn:1-comp] This is multicomposition of the 1-cells.
[fn:cut-elim] This is "cut elimination"/admissability of multicomposition

**** Technical note: Structural rules
The rules for names given so far imply very strict resource management: Every input resource must be used, exactly once, in order. There are some cases where this is necessary for correctness. Consider the case where a name depends on three input streams that gets instantiated with three pipes each filled sequentially by the same process; the first pipe must be completely read from before the process will start filling the second one, so the name must consume it first, and the data streams can be arbitrarily long so they cannot in general be duplicated. In most cases, however, we can relax this through any combination of the following three operations, called *structural rules*:

[FIG]

*Weakening* lets you ignore some input: the new input wire is a "dummy" that doesn't go anywhere in the original name. *Contracting* lets you duplicate some input: the single wire copies[fn:ref] the resource it's instantiated with and sends one copy over each relevant wire in the original name. *Exchanging*, visualized by crossing wires, lets you reorder inputs: the input on the new left wire is forwarded on to the original right wire etc.

By default, all inputs and outputs are eligible for all three rules. On a case by case basis we can conceptually annotate given inputs or outputs with *substructural restrictions*. Marking an output as *relevant* indicates that the result must be used and thus can't be connected to an input wire created through weakening; marking an input as relevant indicates that the name does in fact use that input. Marking an output as *affine* indicates that the result can't be copied[fn:proj] and thus can't be connected to an input wire covering a contraction; marking an input as affine indicates that the name does not duplicate that input. Marking an output as *ordered* indicates that nothing before it can be used once it's used (if ever) and it can't be used once something after has been used and thus can't be connect to an exchanged input wire; marking an input as ordered indicates that the name does not reorder resources around that input[fn:one-sided].

In addition to ensuring correctness in rare cases, these annotations can also be used for optimization. If an input is marked relevant, the caller (or general substitution mechanism) might eagerly prepare the resource for consumption (e.g. starting a socket-activated service) rather than waiting for it to be used, since it will eventually. If an input is marked affine, the caller might garbage collect the resource once it's used. If it's marked ordered, all resources before the input in question can be discarded/preparations stopped once the input is used, and the input itself discarded once something after it is.
...
[fn:proj] There are some subtleties with bundles of affine resources. We want to be able to project out each of the bundled resources, not only one, but each only once. Categorically, I think this should follow from the functoriality of whatever tensor we're using for bundling, but I haven't yet worked out the details in a multicategory.
**** TODO figures
*** Reductions
...
Reductions can be chained together[fn:vert]: If name A reduces to name B and name B reduces to name C, we have a reduction from A to C. Reductions also compose with substitutions[fn:horiz]:

[FIG]

Reductions can change the input requirements; we can drop, duplicate, or rearrange wires[fn:red-struct] so long as we respect substructral restrictions[fn:red-substruct]. Reductions can also *downcast* output types into a more specific type, if we know that the output in the specific cases we've isolated will actually be the right kind of resource. Together, these capabilities allow us to flexibly build names that reuse other names for their work and make that reuse visible to the system as a whole. For example, we could build a TTL cache combinator that takes some name and produces a new name that takes all the same inputs plus the current time and cache state, and either reduces to some named resource if we've run this name recently enough or reduces to the underlying name with the remaining inputs if we haven't (and captures the result for next time)[fn:ml]. Or all of our names that deal with files could delegate the actual file storage to some content-based names and downcast the results to an appropriate specific kind of file, allowing us to identify two different names that result in a file with the same contents as being the same.

The rules reductions must follow are tedious to state in detail, but they all follow from an intuitive notion of being "well-behaved". Reductions must preserve determinism, for example, and the order that we compose reductions and substitutions in doesn't matter. We also have the expected cancellations: downcasting followed by coercing back to the more general type reduces to not having touched the relevant output in the first place.

[fn:2-cell] The (mono) 2-cells.
[fn:vert] Vertical composition
[fn:horiz] Horizontal composition
[fn:red-struct] In fact, our structural operations come with the obvious reductions
...
+ Forwarding
+ Static/subnamespace (e.g. header lookup)
*** Prior work
+ CAS (git, IPFS)
+ Nix
  + Enforcement note
