...
Thus, ~vim?version=8.2~ would be a syntacitcally valid name identifier.
...
1. An optional input name, which is an identifier string, followed by a ~:~ and a space, such as ~bash:~ . If omitted, there must either only be one input to substitute or the relevant input is identified positionally. We will probably eventually support nested input names, referring to inputs of inputs.
2. A name reference.
3. An optional output specifier, which is an identifier string, preceded by a ~.~. If omitted, there must either only be one output of the relevant name or the desired output is identified by the context.
...
In full generality, a name can be an arbitrary directed graph with named edges, including cycles[fn:glibc]. To support this, we need syntax for a declaring a node in the graph, together with its inputs. Each *node declaration* consists of:
